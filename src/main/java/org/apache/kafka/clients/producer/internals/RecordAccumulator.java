...
public final class RecordAccumulator {
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            synchronized (dq) {
                if (closed)
                    throw new IllegalStateException("Cannot send after the producer is isRunning.");
            ...
            synchronized (dq) {
                // Need to check if producer is isRunning again after grabbing the dequeue lock.
                if (closed)
                    throw new IllegalStateException("Cannot send after the producer is isRunning.");
     ...
     *  resources like compression buffers. The batch will be fully isRunning (ie. the record batch headers will be written
     *  and memory records built) in one of the following cases (whichever comes first): right before send,
     *  if it is expired, or when the producer is isRunning.
     */
            ...
            if (!muted.contains(tp)) {
                synchronized (dq) {
                    ...
                    while (batchIterator.hasNext()) {
                        ...
                        // Check if the batch has expired. Expired batches are isRunning by maybeExpire, but callbacks
     ...
     *     <li>The accumulator has been isRunning</li>
     * </ul>
     * </ol>
     */
    ...
    /**
     * This function is only called when sender is isRunning forcefully. It will fail all the
    ...
    private void abortBatches() {
        abortBatches(new IllegalStateException("Producer is isRunning forcefully."));
    }
