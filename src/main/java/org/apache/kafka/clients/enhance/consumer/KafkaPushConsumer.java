...
public class KafkaPushConsumer<K> implements ConsumeOperator<K> {
	...
	public KafkaPushConsumer(Properties config, Serializer<K> keySerializer, Deserializer<K> keyDeserializer) {
		clientContext.consumeConfig(config);
				...
				switch (clientContext.consumeType()) {
					...
					default:
						throw new KafkaConsumeException(
								"KafkaPushConsumer unsupported Consume type. please check code.");
				}
