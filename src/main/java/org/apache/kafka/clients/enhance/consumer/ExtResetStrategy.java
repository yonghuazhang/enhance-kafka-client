...
public enum ExtResetStrategy {
	RESET_FROM_EARLIEST(OffsetResetStrategy.EARLIEST, "earliest", INVALID_TIMESTAMP), RESET_FROM_LATEST(
			OffsetResetStrategy.LATEST, "latest", INVALID_TIMESTAMP), RESET_FROM_TIMESTAMP(OffsetResetStrategy.LATEST,
			"timestamp", INVALID_TIMESTAMP), RESET_NONE(OffsetResetStrategy.NONE, "none", INVALID_TIMESTAMP);
