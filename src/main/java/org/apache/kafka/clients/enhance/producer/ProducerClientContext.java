		...
		for (String key : originalConfig.keySet()) {
			try {
				if (producerKeys.contains(key)) {
					...
					logger.info("[ProducerClientContext] Invalid property: key = [{}], value = [{}].", key,
							originalConfig.get(key));

				}
			} catch (Exception ex) {
				logger.warn("[ProducerClientContext] Invalid property type: key = [{}], value = [{}].", key,
						originalConfig.get(key));
			}
		}
